...
void MyRigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix) {
	...
	m_v3CenterG = static_cast<vector3>(m_m4ToWorld * vector4(m_v3CenterL, 1.0f));
}
//Allocation
...
{
	Init();
	uint vertexCount = a_pointList.size();
	//checks for valid input
	if (vertexCount < 3)
		return;

	//calculates min & max for x, y, and z
	...
	for (uint i = 0; i < vertexCount; i++) {
		...
		if (fDistance > m_fRadius)
			m_fRadius = glm::distance(m_v3CenterL, a_pointList[i]);
	}
...
void MyRigidBody::AddToRenderList(void)
{
	...
	m_pMeshMngr->AddWireCubeToRenderList(glm::translate(m_m4ToWorld, m_v3CenterL) * glm::scale(m_v3HalfWidth), C_BLUE_CORNFLOWER, RENDER_WIRE);
}
